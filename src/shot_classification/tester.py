...
# paths
model_path = "serialized_models/shot_classifier.keras"
video_path = "data/court-level-videos/videoplayback.mp4"

# load model
shot_classifier = load_model(model_path)
...
while True:
    ...
    if len(frame_buffer) >= seq_len: # buffer has reached its limit
        ...
        frame_buffer = frame_buffer[int(seq_len/2):]  # keep only the remaining frames
